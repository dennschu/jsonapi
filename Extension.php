...
class Extension extends \Bolt\BaseExtension {
  ...
  public function initialize() {
    ...
    /** Taxonomy json api endpoints */
    $this->app->get( $this->base . "/taxonomy", [ $this, 'jsonapi_taxonomy' ] )
        ->bind( 'jsonapi_taxonomy_list' );
   ...
   * @param null|string $taxonomyType
   *
   * @return Symfony\Component\HttpFoundation\Response
   */
  public function jsonapi_taxonomy( Request $request, $taxonomyType = null ) {
    $this->request = $request;

    $taxonomyData = $this->app['config']->get( "taxonomy" );

    if ( ! array_key_exists( $taxonomyType, $taxonomyData ) && ! is_null( $taxonomyType ) ) {
      return $this->responseNotFound(
          [
              'detail' => "TaxonomyType with name [$taxonomyType] not found."
          ]
      );
    }

    $taxonomyKeys = is_null( $taxonomyType ) ? array_keys( $taxonomyData ) : [ $taxonomyType ];
    $responseData = [ 'data' => [ ] ];

    foreach ( $taxonomyKeys as $taxonomyKey => $taxonomyValue ) {
      $taxonomyOptions = [ ];

      if ( isset( $taxonomyData[ $taxonomyType ]['options'] ) && is_array( $taxonomyData[ $taxonomyType ]['options'] ) ) {
        foreach ( $taxonomyData[ $taxonomyType ]['options'] as $key => $value ) {
          $taxonomyOptions[] = [
              'key'   => '/' . $taxonomyData[ $taxonomyType ]['slug'] . '/' . $key,
              'value' => $value
          ];
        }
      }
      ...
      $responseData['data'][] = [
          'id'         => $taxonomyData[ $taxonomyType ]['slug'],
          'type'       => isset( $taxonomyData[ $taxonomyType ]['behaves_like'] ) ? $taxonomyData[ $taxonomyType ]['behaves_like'] : false,
          'attributes' => [
              'name'          => $taxonomyData[ $taxonomyType ]['name'],
              'slug'          => $taxonomyData[ $taxonomyType ]['slug'],
              'singular_name' => $taxonomyData[ $taxonomyType ]['singular_name'],
              'singular_slug' => $taxonomyData[ $taxonomyType ]['singular_slug'],
              'has_sortorder' => $taxonomyData[ $taxonomyType ]['has_sortorder'],
              'options'       => $taxonomyOptions
          ],
          'links'      => [
              'self' => $this->basePath . '/taxonomy/' . $taxonomyData[ $taxonomyType ]['slug']
          ]
      ];
    }
